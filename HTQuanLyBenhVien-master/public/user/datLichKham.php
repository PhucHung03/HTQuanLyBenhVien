        ...
        if ($ngayKham < $currentDate || ($ngayKham == $currentDate && $gioKham <= $currentTime)) {
            ...
            try {
                ...
                $queryCheckLich = $pdo->prepare("
                    SELECT gioKham
                    FROM lichhenkham
                    WHERE maBacSi = ? AND ngayKham = ?
                ");
                $queryCheckLich->execute([$bacSi, $ngayKham]);
                $existingAppointments = $queryCheckLich->fetchAll(PDO::FETCH_ASSOC);

                if (count($existingAppointments) > 0) {
                    // Hiển thị các giờ đã được đặt
                    $takenHours = array_map(function($appointment) {
                        return $appointment['gioKham'];
                    }, $existingAppointments);

                    // Chuyển mảng giờ đã đặt thành chuỗi để hiển thị
                    $takenHoursString = implode(", ", $takenHours);

                    // Thông báo giờ đã được đặt
                    echo "<script>alert('Giờ khám đã được đặt cho bác sĩ vào ngày $ngayKham. Các giờ đã được đặt: $takenHoursString. Vui lòng chọn giờ khác.');</script>";
                } else {
            ...
            <div class="row">
                    <div class="header__top">
                        <ul>
                            ...
                            <a href="index.php?url=dang-xuat">Thoát !</a></li>
                        </ul>
                    </div>
            </div>
        </div>
